...
router.get('/profile', auth, async (req, res) => {
  ...
  const offeredRow = await get('SELECT COUNT(*) AS c FROM rides WHERE user_id=?', [req.user.id])
  const joinedRow = await get("SELECT COUNT(*) AS c FROM requests WHERE rider_id=? AND status='accepted'", [req.user.id])

  const avgRow = await get('SELECT AVG(stars) AS avg, COUNT(*) AS c FROM ratings WHERE to_user_id=?', [req.user.id])
  ...
  const ridesOffered = offeredRow.c
  const ridesJoined = joinedRow.c
...
router.put('/profile', auth, async (req, res) => {
  ...
  await run('UPDATE users SET name=?, phone=?, bio=? WHERE id=?', [
    name.trim(),
    phone.trim(),
...
router.put('/profile/avatar', auth, async (req, res) => {
  ...
  await run('UPDATE users SET avatar=? WHERE id=?', [trimmed, req.user.id])
  const user = await get('SELECT * FROM users WHERE id=?', [req.user.id])
  res.json({ user: publicUser(user) })
})
